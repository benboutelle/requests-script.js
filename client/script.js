...
sayHelloButton.addEventListener(`mouseout`,originalColors)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    let request =  axios.get(`http://localhost:3000/repeat/jesus idk`)
    request.then(res => console.log(res.data))
    let repeatText =  document.getElementById("repeat-text")
    repeatText.textContent = request
...
// CODE HERE
const ayFunction = () => {
    axios.get (`http://localhost:3000/query-test?yoyoyo&yaaaayaaa`)
        .then(res => {
            console.log(res.data)
        })
}
document.getElementById(`query-button`).addEventListener(`click`, ayFunction)
...
let createFood = (event) => {
    ...
    let foodInput = document.querySelector(`#form-input`).value
    const body ={
        newFood: foodInput
    }
...
document.querySelector(`#form-button`).addEventListener(`click`, createFood)